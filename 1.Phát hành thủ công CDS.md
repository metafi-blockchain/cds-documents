...
# 3. Luồng đưa CDs lên markets

```mermaid
sequenceDiagram
    autonumber

    participant Seller as Seller (CD Holder)
    participant Buyer as Buyer
    participant UI as Marketplace UI / DApp
    participant MP as Marketplace Contract
    participant TS as CDS Token Contract
    participant CS as Custodian Service (PoR/Settlement)
    participant FF as FireFly / Gateway
    participant CH as Fabric Chaincode (if Fabric-X)

    %% ================
    %% SELLER LISTS CDS
    %% ================

    Seller->>UI: Tạo Listing (assetId, amount, price)
    UI->>MP: createListing(assetId, amount, price)
    MP->>TS: transferFrom(Seller → Marketplace, amount)
    TS-->>MP: Token locked
    MP-->>UI: Listing Created

    %% ==================
    %% BUYER PURCHASES CDS
    %% ==================
    Buyer->>UI: Buy CDS (listingId)
    UI->>MP: buy(listingId) + payment

    %% -- COMPLIANCE GATE --
    MP->>CS: verifyCompliance(buyer, assetId)
    CS-->>MP: OK

    %% =============
    %% SETTLEMENT
    %% =============
    par Transfer Token On-chain
        MP->>TS: transfer(Marketplace → Buyer, amount)
        TS-->>MP: transfer confirmed
    and Payment Settlement
        MP->>CS: settlePayment(buyer → seller)
        CS-->>MP: settlement complete
    end

    %% =============
    %% FINALIZATION
    %% =============
    MP-->>UI: Trade Executed
    UI-->>Buyer: Notify Purchased
    UI-->>Seller: Notify Sold
```
